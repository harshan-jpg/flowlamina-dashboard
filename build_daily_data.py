...
def main():
    ...
    for pg in notion_pages(ntoken, TIMELOG_DS):
        ...
        if not d:
            continue
        d = d[:10]
        # Native model (2026-07-23): the 📊 Daily totals journal row carries a WRITTEN
        # per-day Time total (so Notion can sum days natively per week/month). Work rows
        # remain the source of truth for hours here — skip journal-row Time or we double-count.
        is_daily_row = (prop(p.get("Name")) or "").startswith("📊 Daily totals")
        hrs = 0 if is_daily_row else (prop(p.get("Time")) or 0)
        days[d]["hours"] += hrs
        ...
        days[d]["youtube_videos"] += (prop(p.get("YouTube Videos")) or 0)     # count of videos made; on the daily-totals rows
    ...
    if FINANCE_DS:
        for pg in notion_pages(ntoken, FINANCE_DS):
            ...
            if prop(p.get("Direction")) == "Expense":
                b["expenses"] += abs(val)
            # Owner's drawings (money Harshan pays himself) are Direction=Transfer, so they
